...
code_package_id=$(cpdctl code-package create --space-id $qa_space_id --file-reference code_package/dbscan-credit-risk.zip --name "$code_package_name" --output json | jq -r '.asset_id')
